...
from model import define_model, multi_loss, my_callback, my_metric, my_class_weight, arcface
...
def compare_predict_metric(master_metric_list, master_name_list, dst, model,
                            TTA='flip',
                            TTA_rotate_deg=0,
    ...
    """
    c_metric = base_predict.predict_tta(model, dst,
                                        TTA=TTA,
...
def predict_metric_image_tfapi(PATH_TO_CKPT, PATH_TO_LABELS, PATH_TO_IMAGE, NUM_CLASSES, output_dir,
                        ...
                        master_metric_list=None, master_name_list=None,
                        master_metric_list_2=None, class_min_score_thresh_2=0.0, class_unit=11
                        ):
    """
    tfAPIでpredict
    ...
    if class_model is not None:
        ...
        for box, score in zip(boxes, scores):
            ...
            #class_conf, class_label = compare_predict_metric(master_metric_list, master_name_list, dst, class_model, target_size=(model_height, model_width))
            #print('master:', class_conf, class_label) # cosとラベル確認用
            ########################### 分類モデルのpredict再実行するかの閾値 ###########################
            #if class_conf < class_min_score_thresh_2 and
            if master_metric_list_2 is not None:
                c_metric = base_predict.predict_tta(class_model, dst,
                                                    TTA='flip',
                                                    TTA_rotate_deg=0,
                                                    TTA_crop_num=0, TTA_crop_size=[224, 224],
                                                    preprocess=1.0/255.0,
                                                    resize_size=[model_height, model_width])

                # trainの一部もマスター画像として、切り出し画像をmetric learning modelでpredict
                cos_sim_mean_list = []
                for i,m_name in enumerate(master_name_list):

                    # trainの一部のマスター画像は1クラス11枚単位なので11枚とる！！！！(class_unit=6)
                    # D:\work\JT_SIGNATE_Contest\01_classes\Images\train_10\test_validation
...
def predict_metric_dir_tfapi(PATH_TO_CKPT, PATH_TO_LABELS, PATH_TO_IMAGE_DIR, NUM_CLASSES, output_dir,
                            ...
                            class_model=None, dict_class={0.0:"Car"}, class_min_score_thresh=0.5, model_height=331, model_width=331,
                            master_metric_list=None, master_name_list=None,
                            master_metric_list_2=None, class_min_score_thresh_2=0.6, class_unit=11
                            ):
    """
    ...
    for path in pbar:
        ...
        boxes, scores, classes, img_RGB = predict_metric_image_tfapi(PATH_TO_CKPT, PATH_TO_LABELS, path, NUM_CLASSES, output_dir,
                                                                    ...
                                                                    master_metric_list=master_metric_list, master_name_list=master_name_list,
                                                                    master_metric_list_2=master_metric_list_2, class_min_score_thresh_2=class_min_score_thresh_2, class_unit=class_unit
                                                                    )
        ymin_list = []
        xmin_list = []
        ymax_list = []
